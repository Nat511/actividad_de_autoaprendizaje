...
# # Función para detectar y eliminar valores atípicos usando el rango intercuartil (IQR)
# def eliminar_valores_atipicos(df, columns):
#     df_sin_atipicos = df.copy()
#     for column in columns:
#         Q1 = df_sin_atipicos[column].quantile(0.25)
#         Q3 = df_sin_atipicos[column].quantile(0.75)
#         IQR = Q3 - Q1
...
# # Seleccionar columnas para análisis
# columnas_analisis = ["value", "co2_saved", "energy_efficiency", "year"]

# # Gráficos de distribución (curvas de Gauss) antes de eliminar outliers
# st.subheader("Gráficos antes de eliminar valores atípicos")
# fig, ax = plt.subplots(1, len(columnas_analisis), figsize=(15, 5))
# for i, col in enumerate(columnas_analisis):
#     sns.histplot(df[col], kde=True, ax=ax[i])
#     ax[i].set_title(f'Distribución {col} (Original)')
# st.pyplot(fig)
...
# # Gráficos de boxplot antes de eliminar outliers con Plotly Express
# cols = st.columns(len(columnas_analisis))
# for i, col in enumerate(columnas_analisis):
#     fig_box = px.box(df, y=col, title=f"Boxplot {col} (Original)")
#     cols[i].plotly_chart(fig_box)

# # Eliminar valores atípicos en todas las columnas seleccionadas
# df_sin_atipicos = eliminar_valores_atipicos(df, columnas_analisis)
...
# for i, col in enumerate(columnas_analisis):
#     fig_box_sin = px.box(df_sin_atipicos, y=col, title=f"Boxplot {col} (Sin Outliers)")
#     cols[i].plotly_chart(fig_box_sin)
